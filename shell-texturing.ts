...
let geometry: THREE.BufferGeometry = new THREE.PlaneGeometry(1, 1).rotateX(-Math.PI / 2);
...
function render(ms: number) {
	for (const shell of shells) {
		const time = (shell.material as THREE.ShaderMaterial).uniforms.time.value = ms / 1000;
	}
...
function geomChanger() {
	if (controls.geometry === 'Plane') {
		...
		geometry = (suzanne.children[0] as THREE.Mesh).geometry.scale(0.5, 0.5, 0.5);
	}
...
function matChanger () {
	for ( const e in controls ) {
		for (const shell of shells) {
			const shader = shell.material as THREE.ShaderMaterial;
